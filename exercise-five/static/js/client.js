...
window.onload = function () {
  ...
  async function runQuery(queryPath) {
    ...
    // Stop any running animation from previous views
    if (animationId) {
      ...
      switch (queryPath) {
        ...
        case "three": {
          displayWave(resJSON, {
            ...
            key: "after_mood", list: resJSON.moods, mode: "simple"
          });
          break;
        }
        case "four": {
          displayWave(resJSON, {
            bg: "#1a1a1a",
            key: "event_name", list: resJSON.events, mode: "helix"
          });
          break;
        }
        ...
        case "five": {
          displayWave(resJSON, {
            ...
            key: "day", list: resJSON.days, mode: "split", ampProp: "event_affect_strength"
          });
          break;
        }
        case "six": {
          displayWave(resJSON, {
            bg: "#2c3e50",
            key: "weather", list: resJSON.weather, mode: "vertical"
          });
          break;
        }
        default: {
...
//*** drawing a grid ****/
  ...
  function displayWave(data, cfg) {
    ...
    (cfg.list || []).forEach((item, i) => {
      colorMap[item] = `hsl(${(i * 360) / cfg.list.length}, 70%, 50%)`;
    });

    let wrapper = document.querySelector("#parent-wrapper");
    wrapper.style.background = cfg.bg;
    let container = document.querySelector("#childOne");
    
    // create points
    resultSet.forEach(d => {
      dataPoints.push(new myDataPoint(
        d.dataId, d.day, d.weather, d.start_mood, d.after_mood,
        d.after_mood_strength, d.event_affect_strength, d.event_name,
        colorMap[d[cfg.key]], container, cfg.mode === "split" ? "point" : "point_two"
      ));
    });
    ...
    function animate() {
      ...
      dataPoints.forEach((dp, i) => {
        ...
        // determine amplitude based on configuration or default
        const amplitude = cfg.ampProp ? dp[cfg.ampProp]  * 15 : 50; // if ampProp is a string, use it as property name
        
        if (cfg.mode === "vertical") {
          // vertical wave logic
          const verticalY = 50 + i * spacing;
          const verticalFrequency = 0.05;
          const verticalWaveWidth = 200;
          
          y = verticalY;
          x = centerX + Math.sin(y * verticalFrequency + time) * verticalWaveWidth;
        } else {
          // horizontal wave logic
          const horizontalX = (i * spacing) % windowWidth;
          const currentRow = Math.floor((i * spacing) / windowWidth);
          
          // calculate base y position based on row and mode
          const rowHeight = (cfg.mode === "split" ? 400 : 250);
          let baseY = currentRow * rowHeight + centerY;
          
          // adjust for split mode (monday vs tuesday)
          if (cfg.mode === "split") {
            const isMonday = dp.day === "Monday";
            const splitOffset = isMonday ? -100 : 100;
            baseY += splitOffset;
          }
          
          // adjust phase for helix mode
          const isHelixOddIndex = (cfg.mode === "helix" && i % 2 !== 0);
          const phaseShift = isHelixOddIndex ? Math.PI : 0;
          
          // calculate sine wave offset
          const frequency = 0.1;
          const sineOffset = Math.sin(i * frequency + time + phaseShift) * amplitude;
    ...
    animate();
    container.style.height = (cfg.mode === "vertical" ? resultSet.length * 15 + 200 : 2000) + "px";
  }
